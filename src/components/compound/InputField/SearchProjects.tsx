...
export default function SearchSelect({
  ...
  function handleFindExistOption(prevState: string[], option: string) {
    const exist = prevState.find((prevStateOption) => prevStateOption === option);
    if (exist) {
      return prevState;
    }
  ...
  return (
    ...
    >
      ...
      <div
        // eslint-disable-next-line prettier/prettier
        className={`absolute bg-primary-white z-50 h-auto w-full flex-col flex translate-y-[-7px] transform rounded-b-[.650rem] -ml-1
                ...
                <button
                  ...
                  onClick={() => {
                    if (option !== 'Nenhum resultado') {
                      setValue((prevState) => {
                        return handleFindExistOption(prevState, option);
                      });
                    }
                    setTimeout(() => {
                      setFocus(nameField);
